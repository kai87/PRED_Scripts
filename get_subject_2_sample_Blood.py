...
out_file_note = open(path + "./Outputs/Blood_note_Kai.txt", "w")
...
in_raw_file = open(path + "./Raw_data/Blood/expression.txt", "r")
...
in_file = open(path + "./Raw_data/Blood/PAXgene Microarray Annotation for U-BIOPRED Upload 20140410.txt", "r")
out_file = open(path + "./Outputs/Blood_sub2sample_kai.txt", "w")
...
for line in in_file:
	...
	if sample_id in sample2subject_mapping:
		...
		original_subject_id = line.split("\t")[3]
		if subject_id != original_subject_id:
			print "(%s, %s)" %(subject_id, original_subject_id)
			
		if subject_id in subjects_for_uploading:		
			outline = "May2014_UBIO" + "\t" + "\t" + subject_id + "\t" + sample_id + "\t" + "GPL570" + "\t" + "Blood" + "\t" + "Transcriptomics" + "\t" + "\t" + "OMICS_DATA+ATTR1+Baseline_Visit+TISSUETYPE+PLATFORM" + "\n"
			out_file.write(outline)
