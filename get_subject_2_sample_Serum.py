...
out_file_note = open(path + "/Outputs/Serum_note.txt", "w")
...
in_file = open(path + "/Raw_data/Serum/SERUM Proteomics provisional IA up to 60%missingness_Fmol.txt", "r")
out_file = open(path + "/Outputs/Serum_sub2sample_kai.txt", "w")
...
for line in in_file:
	sample_id = line.split("\t")[0].replace("_Avg", "")
	
	if sample_id in sample2subject_mapping:
		subject_id = sample2subject_mapping[sample_id]
			
		if subject_id in subjects_for_uploading:		
			outline = "May2014_UBIO" + "\t" + "\t" + subject_id + "\t" + sample_id + "\t" + "GPL570" + "\t" + "Serum" + "\t" + "Proteomics" + "\t" + "\t" + "OMICS_DATA+ATTR1+Baseline_Visit+TISSUETYPE+PLATFORM" + "\n"
			out_file.write(outline)
